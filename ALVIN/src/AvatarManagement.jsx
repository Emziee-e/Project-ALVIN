        ...
        <main className="flex-1 w-full md:ml-60 lg:ml-64 bg-white overflow-hidden flex flex-col h-screen min-w-0">
          ...
          <div className="flex-1 overflow-y-auto w-full">
            ...
            <section className="text-left mb-8 md:mb-12">
              <h1 className="font-[GeistSans,sans-serif] text-4xl md:text-6xl font-bold tracking-tighter text-black mb-2">
                Avatar Management
              </h1>
              <p className="text-gray-500 font-[SF_Pro_Display,sans-serif] max-w-2xl text-base md:text-lg">
                Manage AI avatars and voice settings. Choose a primary avatar and voice model. <span className="font-bold">Selected models will be applied to all users.</span>
              </p>
            </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-start">
              ...
              <div className="lg:col-span-8 space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3">
                  <h3 className="font-[GeistSans,sans-serif] text-xl md:text-2xl font-bold uppercase tracking-tight">
                    Avatar Models
                  </h3>
                </div>
                    ...
                    >
                      ...
                      <div className="aspect-[4/5] overflow-hidden bg-gradient-to-br from-gray-200 to-gray-300 relative">
                        <div className="w-full h-full flex items-center justify-center">
                          <div className={`text-6xl font-[GeistSans,sans-serif] font-black transition-all duration-700
                      ...
                      <div className="absolute bottom-0 left-0 p-4 md:p-6 w-full">
                        <div className="flex justify-between items-end gap-2">
                          <div className="min-w-0">
                            {model.selected && (
                              <span className="text-[#862334] font-[SF_Pro_Display,sans-serif] text-[10px] uppercase tracking-widest mb-1 block bg-white/90 px-1.5 py-0.5 w-fit">
                                Active Selection
                              </span>
                            )}
                            <h4 className="text-lg md:text-2xl font-[GeistSans,sans-serif] font-bold text-white truncate">
                              {model.name}
                            </h4>
                            <p className="text-gray-300 text-xs md:text-sm font-[SF_Pro_Display,sans-serif] truncate">{model.sub}</p>
                          </div>
                          {model.selected ? (
                            <div className="bg-[#862334] text-white p-2 flex-shrink-0">
                              <Icon name="check_circle" filled className="text-lg" />
                            </div>
                          ) : (
                            <button
                              onClick={e => { e.stopPropagation(); selectModel(model.id); }}
                              className="bg-white/90 text-black px-2 md:px-4 py-1.5 md:py-2 text-[10px] md:text-xs font-[GeistSans,sans-serif] font-bold uppercase tracking-widest hover:bg-[#862334] hover:text-white transition-all flex-shrink-0 whitespace-nowrap"
                            >
                              Select
                            </button>
                          )}
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                {/* Voice panel */}
                <div className="bg-gray-50 border border-gray-200 p-5 md:p-8">
                  <h3 className="font-[GeistSans,sans-serif] text-xl md:text-2xl font-bold uppercase tracking-tight mb-6 md:mb-8">
                    Voice Models
                  </h3>
                  ...
                  <div className="space-y-5 md:space-y-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-[SF_Pro_Display,sans-serif] uppercase tracking-widest text-[#862334] block">
                        Primary Voice
                      </label>
                      <div className="relative">
                        <select
                          ...
                          className="w-full bg-gray-100 border-none text-black py-3 md:py-4 pl-4 pr-12 focus:ring-1 focus:ring-[#862334] appearance-none font-[SF_Pro_Display,sans-serif] outline-none text-sm"
                        >
                          <option>ALVIN VOICE 1 (Alt-Alto)</option>
                          <option>ALVIN VOICE 2 (Deep Bass)</option>
                          <option>ALVIN VOICE 3 (Neutral Mid)</option>
                        </select>
                        <Icon name="expand_more" className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-gray-500" />
                      </div>
                    </div>

                    {/* Preview clips */}
                    <div className="bg-white p-4 md:p-6 space-y-4 border border-gray-100">
                      <div className="flex items-center justify-between">
                        <span className="font-[GeistSans,sans-serif] font-bold text-sm tracking-widest uppercase">Preview Clips</span>
                      </div>
                      <div className="space-y-2">
                        {voiceClips.map((clip, i) => (
                          ...
                          >
                            <div className="flex items-center gap-3 md:gap-4">
                              ...
                              >
                                <Icon name={playing === i ? "pause" : "play_arrow"} filled className="text-base" />
                              </button>
                              <div>
                                <p className="text-sm font-bold text-black">{clip.label}</p>
                                <p className="text-[10px] text-gray-400 uppercase font-[SF_Pro_Display,sans-serif]">{clip.duration}</p>
                              </div>
                            </div>
                            {/* Waveform bars */}
                            <div className={`flex gap-0.5 items-end transition-opacity ${playing === i ? "opacity-100" : "opacity-20 group-hover:opacity-80"}`}>
                              {[4, 6, 3, 5, 7, 4].map((h, j) => (
                                <div
                                  key={j}
                                  className="w-1 bg-[#862334] rounded-sm"
                                  style={{ height: `${h * 3}px` }}
                                />
                              ))}
                            </div>
                    ...
                    {/* CTA */}
                    <button className="w-full bg-[#862334] text-white py-4 md:py-5 font-[GeistSans,sans-serif] font-bold text-sm uppercase tracking-[0.2em] hover:bg-[#ffb003] hover:-translate-y-0.5 transition-all shadow-[0_10px_30px_rgba(134,35,52,0.2)]">
                      Update Voice Model 
                    </button>
                  </div>
                </div>
